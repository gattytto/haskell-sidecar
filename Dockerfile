...
RUN cd ${HOME} && \
    cabal update && \
    git clone https://github.com/haskell/ghcide.git && cp stack8102.yaml ghcide/ && rm -f stack88.yaml && cd ghcide && stack install --system-ghc --stack-yaml stack88.yaml && cd .. && \
    ...
    echo "- exceptions-0.10.4" >> ${HOME}/ghci-dap/stack.yaml && \
